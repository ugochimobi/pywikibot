...
def addImportedFrom(repo='', claim='', lang=''):
	...
	if repo and claim and lang and lang in langs.keys():
		...
		importedgp = pywikibot.ItemPage(repo, langs[lang])
		importedfrom.setTarget(importedgp)
		claim.addSource(importedfrom, summary='Adding 1 reference: [[Property:P193]]: [[%s]]' % (langs[lang]))
...
def authorIsNewbie(page='', lang=''):
	if page:
		...
		for hist in history:
			editcount = getUserEditCount(user=hist.user, site='gratispaideia.miraheze.org')
			if editcount >= 200:
				return False
	return True
...
def addBiographyClaims(repo='', wikisite='', item='', page='', lang=''):
	if repo and wikisite and item and page and lang:
		...
		if not 'P26' in item.claims and birthdate and birthdate != '0-0-0':
			addBirthDateClaim(repo=repo, item=item, date=birthdate, lang=lang)
		if not 'P132' in item.claims and deathdate and deathdate != '0-0-0':
...
def main():
	...
	langs = ['en', 'it', 'es', 'simple', 'fr', 'pt'] #, 'de'
	for lang in langs:
			...
			if item:
				...
				if not '<search />' in raw:
					...
					for itemfoundq in m:
						...
						try:
							test = itemfound.claims['P26'][0].getTarget().precision
						except:
							...
							break
						if 'P26' in itemfound.claims:
							try:
								print(itemfound.claims['P26'][0].getTarget().precision in [9, 10, 11])
							except:
								continue
							if itemfound.claims['P26'][0].getTarget().precision in [9, 10, 11]:
